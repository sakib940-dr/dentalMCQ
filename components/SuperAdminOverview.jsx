...
export default function SuperAdminOverview() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      // Rewritten from a per-subject loop (up to 30 subjects × 3
      // sequential queries each = up to 90 round-trips) into 3 bulk
      // queries total, then joined together in memory.
      ...
      for (const s of subjectsToCheck) {
        ...
        if (!hasAnySubcat) {
          subjectAttention.push({ type: 'subject', name: s.name, reason: 'No chapters set up yet' });
          continue;
        }
        const count = questionCountBySubject.get(s.id) || 0;
        if (count < 5) {
          subjectAttention.push({ type: 'subject', name: s.name, reason: `Only ${count} question${count === 1 ? '' : 's'}` });
        }
      }
      if (cancelled) return;
